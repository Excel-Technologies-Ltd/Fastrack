...
DROP PROCEDURE IF EXISTS GetAgentByName$$

CREATE PROCEDURE GetAgentByName(
...
BEGIN
    ...
    (
        ...
        (
            ...
            -- Import Air
            SELECT
                TRIM(COALESCE(IAHB.`agent`, '')) AS AgentName
            ...
            -- Import D2D
            SELECT
                TRIM(COALESCE(ID2D.`agent`, '')) AS AgentName
            ...
            -- Export Sea
            SELECT
                TRIM(COALESCE(ESHB.`delivery_agent`, '')) AS AgentName
            ...
            -- Export Air
            SELECT
                TRIM(COALESCE(EAHB.`agent`, '')) AS AgentName
            ...
            -- Export D2D
            SELECT
                TRIM(COALESCE(ED2D.`agent`, '')) AS AgentName
        ...
        WHERE AgentName <> ''
          AND (
                p_search IS NULL
                OR p_search = ''
                OR AgentName LIKE CONCAT('%', p_search, '%')
              )
    ...
    SELECT JSON_OBJECT(
        ...
        COALESCE(
            (
                SELECT JSON_ARRAYAGG(AgentName)
                FROM
                (
                    SELECT DISTINCT AgentName
                    FROM
