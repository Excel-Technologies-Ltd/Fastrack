...
DROP PROCEDURE IF EXISTS GetHblNoByName$$

CREATE PROCEDURE GetHblNoByName(
...
BEGIN
    ...
    FROM
    (
        SELECT DISTINCT HblNo
        FROM
        (
            -- Import Sea
            SELECT
                TRIM(COALESCE(ISHB.`hbl_id`, '')) AS HblNo
            ...
            -- Import Air
            SELECT
                TRIM(COALESCE(IAHB.`hbl_no`, '')) AS HblNo
            ...
            -- Import D2D
            SELECT
                TRIM(COALESCE(ID2D.`hbl_no`, '')) AS HblNo
            ...
            -- Export Sea
            SELECT
                TRIM(COALESCE(ESHB.`hbl_no`, '')) AS HblNo
            ...
            -- Export Air
            SELECT
                TRIM(COALESCE(EAHB.`hbl_no`, '')) AS HblNo
            ...
            -- Export D2D
            SELECT
                TRIM(COALESCE(ED2D.`hbl_no`, '')) AS HblNo
