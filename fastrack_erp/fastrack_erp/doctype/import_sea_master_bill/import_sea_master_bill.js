frappe.ui.form.on('Import Sea Master Bill', {
    ...
    fv_etd: function(frm) {
        if (frm.doc.fv_etd && frm.doc.fv_etd < frm.doc.etd) {
            frappe.msgprint({
                title: __('Invalid ETD Date'),
                message: __('ETD cannot be in the past. Please select a valid date.'),
                indicator: 'red'
            });
            frm.set_value('fv_etd', null);
        }
    }
});
