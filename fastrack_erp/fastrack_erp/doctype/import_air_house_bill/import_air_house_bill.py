...
class ImportAirHouseBill(Document):
	...
	def onload(self):
		...
		for idx, gl_entry in enumerate(gl_entry_list):
			...
			payment_entry_doc.parenttype = "Import Air House Bill"
		...
		# purchase_invoice_list / total_purchase_amount are maintained persistently by
		# doc_events.purchase_invoice.after_submit/on_cancel -- do not rebuild here.
		...
		for item in draft_list:
			...
			draft_invoice.parenttype = "Import Air House Bill"
	...
	def validate_weight(self):
		...
		mbl_doc = frappe.get_doc('Import Air Master Bill', self.mbl_link)
...
def get_draft_sales_and_purchase_invoice_list(house_bill_no):
	"""Get draft invoices for Import Air House Bill"""
	invoice_list = []
