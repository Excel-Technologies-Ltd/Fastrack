...
from erpnext.accounts.report.utils import  get_currency,convert,get_rate_as_at
...
def get_result_as_list(data, filters):
	balance, _balance_in_account_currency,bdt_balance = 0, 0, 0
	...
	for d in data:
		...
		if filters.get("presentation_currency") and filters.get("presentation_currency") != "BDT":
			bdt_balance = get_balance(d, bdt_balance, "bdtdebit", "bdtcredit", )
			d["bdtbalance"] = bdt_balance
		d["balance"] = balance
...
def get_balance(row, balance, debit_field, credit_field):
	...
	return balance
def get_bdt_balance(row, balance, debit_field, credit_field,conversion_rate):
	balance += row.get(debit_field, 0) * conversion_rate - row.get(credit_field, 0) * conversion_rate
...
def get_columns(filters):
	...
	columns = [
		...
		},
		{
			...
			"fieldtype": "Float",
			"width": 130,
			"hidden": 0 if filters.get("presentation_currency") and filters.get("presentation_currency") != "BDT" else 1,
		},
		{
...
def convert_to_presentation_currency(gl_entries, currency_info):
	...
	in `currency_info`. Uses conversion_rate from source documents when available.
	:param gl_entries:
	:param currency_info:
	:return:
	"""
	...
	for entry in gl_entries:
		...
		if len(account_currencies) == 1 and account_currency == presentation_currency:
			...
			conversion_rate = get_document_conversion_rate(
				entry, presentation_currency, company_currency
			)
			
			if conversion_rate:
				# Use document conversion rate
				entry["exchange_rate"] = conversion_rate
				if entry.get("debit"):
					entry["debit"] = debit / conversion_rate
				if entry.get("credit"):
					entry["credit"] = credit / conversion_rate
				entry["bdtdebit"] = debit 
				entry["bdtcredit"] = credit 
				# entry["bdtbalance"] = entry["balance"] * entry["exchange_rate"]
			else:
				# Fallback to standard exchange rate conversion
				...
				entry["bdtdebit"] = debit 
				entry["bdtcredit"] = credit 
				entry["exchange_rate"] = get_rate_as_at(date, presentation_currency, company_currency)
				# entry["bdtbalance"] = entry["balance"] * entry["exchange_rate"]
