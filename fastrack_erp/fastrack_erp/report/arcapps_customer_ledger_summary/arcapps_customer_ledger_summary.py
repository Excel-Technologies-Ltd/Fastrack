...
#
# Duplicate of erpnext.accounts.report.customer_ledger_summary, with an
# extra parallel set of USD-equivalent columns alongside the existing
# (company currency / BDT) ones -- conversion rate for each GL entry comes
# from its own source document (Sales/Purchase Invoice, Payment Entry, or
# Journal Entry -- see get_document_conversion_rate in ArcApps General
# Ledger). Deliberately does NOT fall back to the daily exchange rate table
# (erpnext.accounts.report.utils.get_rate_as_at): with no Currency Exchange
# records configured it silently returns a false 1:1 rate instead of
# failing loudly, which would show a USD figure identical to the BDT one.
# When no per-document rate can be found, the USD amount is 0 rather than
# a fabricated value.
...
def get_usd_conversion_rate(gl_entry, posting_date, company_currency):
	"""BDT-per-USD rate for one GL entry, sourced only from its own document
	(see module docstring for why the daily rate table isn't used)."""
	return get_document_conversion_rate(gl_entry, "USD", company_currency)
...
class PartyLedgerSummaryReport:
	...
	def get_columns(self):
		...
		columns += [
			...
			{
				"label": _("Opening Balance (USD)"),
				"fieldname": "usd_opening_balance",
				"fieldtype": "Float",
				"width": 120,
			},
			{
			...
			{
				"label": _(credit_or_debit_note + " (USD)"),
				"fieldname": "usd_return_amount",
				"fieldtype": "Float",
				"width": 120,
			},
		]
		...
		columns += [
			...
			{
				"label": _("Closing Balance (USD)"),
				"fieldname": "usd_closing_balance",
				"fieldtype": "Float",
				"width": 120,
			},
			{
	...
	def get_data(self):
		...
		for gle in self.gl_entries:
				...
				frappe._dict(
					{
						...
						"invoiced_amount": 0,
						"usd_invoiced_amount": 0,
						"paid_amount": 0,
						"usd_paid_amount": 0,
						"return_amount": 0,
						"usd_return_amount": 0,
						"closing_balance": 0,
						"usd_closing_balance": 0,
			...
			conversion_rate = get_usd_conversion_rate(gle, gle.posting_date, company_currency)
			usd_amount = flt(amount) / conversion_rate if conversion_rate else 0

			self.party_data[gle.party].closing_balance += amount
			self.party_data[gle.party].usd_closing_balance += usd_amount

			if gle.posting_date < self.filters.from_date or gle.is_opening == "Yes":
				self.party_data[gle.party].opening_balance += amount
				self.party_data[gle.party].usd_opening_balance += usd_amount
			else:
				if amount > 0:
					self.party_data[gle.party].invoiced_amount += amount
					self.party_data[gle.party].usd_invoiced_amount += usd_amount
				elif gle.voucher_no in self.return_invoices:
					self.party_data[gle.party].return_amount -= amount
					self.party_data[gle.party].usd_return_amount -= usd_amount
				else:
					self.party_data[gle.party].paid_amount -= amount
					self.party_data[gle.party].usd_paid_amount -= usd_amount
