...
from fastrack_erp.fastrack_erp.report.arcapps_general_ledger.arcapps_general_ledger import (
	get_document_conversion_rate,
)


def get_usd_conversion_rate(row, posting_date, company_currency):
	"""BDT-per-USD rate for one row, sourced only from its own document
	(Sales/Purchase Invoice, Payment Entry, or Journal Entry). Deliberately
	does NOT fall back to the daily exchange rate table
	(erpnext.accounts.report.utils.get_rate_as_at): with no Currency
	Exchange records configured it silently returns a false 1:1 rate
	instead of failing loudly. When no per-document rate is found, the USD
	amount is 0 rather than a fabricated value."""
	return get_document_conversion_rate(row, "USD", company_currency)
...
class ReceivablePayableReport:
	...
	def set_usd_amounts(self, row):
		"""Parallel USD-equivalent columns alongside invoiced/paid/credit_note/
		outstanding, which at this point are in company currency (the default
		display currency of this report). Skipped when in_party_currency is
		on, since those fields hold account/party-currency amounts instead
		and this conversion assumes a company-currency starting point."""
		if self.filters.get("in_party_currency"):
			row.usd_invoiced = row.usd_paid = row.usd_credit_note = row.usd_outstanding = 0.0
			return

		conversion_rate = get_usd_conversion_rate(row, row.posting_date, self.company_currency)
		row.usd_invoiced = flt(row.invoiced) / conversion_rate if conversion_rate else 0.0
		row.usd_paid = flt(row.paid) / conversion_rate if conversion_rate else 0.0
		row.usd_credit_note = flt(row.credit_note) / conversion_rate if conversion_rate else 0.0
		row.usd_outstanding = flt(row.outstanding) / conversion_rate if conversion_rate else 0.0
	...
	def get_columns(self):
		...
		self.add_column(_("Invoiced Amount (USD)"), fieldname="usd_invoiced", fieldtype="Float")
		self.add_column(_("Paid Amount"), fieldname="paid")
		self.add_column(_("Paid Amount (USD)"), fieldname="usd_paid", fieldtype="Float")
		if self.account_type == "Receivable":
			self.add_column(_("Credit Note"), fieldname="credit_note")
			self.add_column(_("Credit Note (USD)"), fieldname="usd_credit_note", fieldtype="Float")
		else:
			# note: fieldname is still `credit_note`
			self.add_column(_("Debit Note"), fieldname="credit_note")
			self.add_column(_("Debit Note (USD)"), fieldname="usd_credit_note", fieldtype="Float")
		self.add_column(_("Outstanding Amount"), fieldname="outstanding")
		self.add_column(_("Outstanding Amount (USD)"), fieldname="usd_outstanding", fieldtype="Float")
